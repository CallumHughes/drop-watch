...
When `RENDER_URL` is configured, new add-flow previews try the browser first
and automatically fall back to HTTP if the browser is unavailable or finds no
price. The actual successful mode is saved with the new listing, so scheduled
checks stay pinned to that winning mode. Existing listings are unchanged;
switch one manually in its settings if you want to change its scheduled check
mode.
