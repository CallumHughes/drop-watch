...
The orchestrator writes the PR, not a sub-agent — you are the only one who saw
every workstream.

1. **Plan-mode recap first.** Before fanning out, run the `visual-recap` skill
   in plan mode against `docs/architecture/primitives.yaml`. The primitives it
   marks as `extends` or `adds` are the shared-file workstreams that must be
   serialized; everything `composes` can fan out freely. Keep the block — it
   becomes the plan half of "plan vs actual".
2. **Open the PR with a real description.** Body = intent and context in your
   own words. Do not paste sub-agent reports.
3. **Attach the recap.** Run `visual-recap` in recap mode against the final
   diff and upsert it into the PR description:

   ```bash
   node .claude/skills/visual-recap/scripts/upsert-recap-block.mjs <pr-number> <block-file>
   ```

   It only rewrites the marker-delimited block, so the description you wrote in
   step 2 survives. Re-run it after any significant push. If the implementation
   drifted from the plan-mode block, fill in "Plan vs actual" — the drift is
   usually the most review-worthy thing in the PR.
4. **Then babysit it** with the `ship-pr` skill.