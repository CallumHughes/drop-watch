...
diagrams), and the PR itself is the storage. Anything reading the recap later
ingests the same marker-delimited block via the GitHub API, so follow the format
exactly.
...
A change touching an invariant from `primitives.yaml` (per-user isolation,
single scheduler, notification-never-fails-a-check, alert dedupe, invite-gated
signup, migration parity) is called out explicitly regardless of
classification.
...
**Classification:** extends — no new primitives; `app-db` gains a column and
the worker reads it.
...
| ---------------- | -------- | ------------------------------------------ |
| `worker-process` | surfaces | composes                                   |
| `app-db`         | storage  | extends — new `products.check_interval`    |
| `migrations`     | storage  | extends — adds `0007_check_interval.sql`   |

### System map

```mermaid
flowchart LR
	worker["worker-process"]:::touched
	queue["job-queue"]:::untouched
	appDb["app-db"]:::extended
	migrations["migrations"]:::extended
	worker --> queue --> appDb
	migrations --> appDb
...
- **System map**: show touched primitives plus their immediate neighbors from
  the map — not all ~19 nodes. Color with the four `classDef` styles above
...
1. Read `docs/architecture/primitives.yaml`.
2. Get the facts: `gh pr view <n> --json baseRefName,headRefName`, then
   ...
   author this session.
3. Map changed paths to primitives via the map's `code` globs (most specific
   glob wins; respect `excludes`); classify each; roll up the overall
   classification. Repo-specific calls:
   - A file under `packages/db/src/migrations/` means `migrations` is
     `extends` at minimum, and the `migration-parity` invariant applies.
   - A changed `packages/*/src/**` file with no matching glob means the map is
     stale — update `primitives.yaml` rather than guessing.
   - Test-only changes (`*.test.ts`, `apps/e2e/**`) never raise the
     classification above `composes` on their own.
